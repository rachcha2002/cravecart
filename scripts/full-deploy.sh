...
# Increased for 8-node cluster (16 vCPUs, 64GB total)
CPU_LIMITS="2.0"
CPU_REQUESTS="1.0"
...
echo -e "\n${YELLOW}[NOTE]${NC} Each service has been allocated ${CPU_REQUESTS} CPU and ${MEMORY_REQUESTS} memory for requests."
echo -e "${YELLOW}[NOTE]${NC} Each service is limited to ${CPU_LIMITS} CPUs and ${MEMORY_LIMITS} memory."
echo -e "${YELLOW}[NOTE]${NC} Your cluster should be configured with 8 nodes for optimal performance with these resources." 