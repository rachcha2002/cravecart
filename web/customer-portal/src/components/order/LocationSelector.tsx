...
import { MapPinIcon, MapIcon, MagnifyingGlassIcon } from '@heroicons/react/24/solid';
    ...
    fetch(`https://maps.googleapis.com/maps/api/geocode/json?address=${encodeURIComponent(query)}&key=${GOOGLE_MAPS_API_KEY}`)
      ...
      .then(data => {
        ...
        if (data.status === 'OK' && data.results && data.results.length > 0) {
          ...
          console.error('Geocoding search error:', data.status, data.error_message);
          setSearchResults([]);
              ...
              {searchResults.map((result, index) => (
                ...
                >
                  ...
                  {result.formatted_address && (
                    <p className="text-sm text-gray-600 dark:text-gray-300">{result.formatted_address}</p>
                  )}
                </li>
              ))}
            </ul>
