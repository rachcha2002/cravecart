...
const validateRestaurantAccess = async (userId, restaurantId) => {
  ...
  try {
    ...
    const response = await axios.get(`${USER_SERVICE_URL}/restaurants/validate-restaurant-access`, {
      params: {
        userId,
        restaurantId
      }
    ...
    if (error.response) {
      ...
      throw new Error(`Access validation failed: ${error.response.data.message || 'Unauthorized'}`);
...
exports.updateMenuItem = async (req, res) => {
  try {
   ...
   // await validateRestaurantAccess(userId, restaurantId);
    ...
    // Update fields
    const updateFields = [
      'name', 'description', 'price', 'discountedPrice', 'imageUrl',
      'calories', 'preparationTime', 'isVegetarian', 'isVegan', 
      'isGlutenFree', 'spicyLevel', 'allergens', 'customizationGroups',
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    // Update fields
    const updateFields = [
      'name', 'description', 'image', 'availabilityTimes', 
      'sortOrder', 'isAvailable'
    ];
